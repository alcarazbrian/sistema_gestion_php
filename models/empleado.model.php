...
class EmpleadoModel
{
    ...
    public function create($dni, $nombre, $apellido, $email, $contraseña, $rol_id, $estado)
    {
        ...
        return $stmt->execute([$dni, $nombre, $apellido, $email, $password_hash, $rol_id, $estado]);
    }
    ...
    public function update($id, $dni, $nombre, $apellido, $email, $contraseña, $rol_id, $estado)
    {
        ...
        if (!empty($contraseña) && substr($contraseña, 0, 4) !== '$2y$') {
            ...
            return $stmt->execute([$dni, $nombre, $apellido, $email, $password_hash, $rol_id, $estado, $id]);
        } else {
            // Es un hash existente o vacío - mantener la contraseña actual
            $stmt = $this->pdo->prepare("UPDATE empleados SET DNI=?, Nombre=?, Apellido=?, Email=?, Rol_ID=?, Estado=? WHERE ID=?");
            return $stmt->execute([$dni, $nombre, $apellido, $email, $rol_id, $estado, $id]);
        }
    }
